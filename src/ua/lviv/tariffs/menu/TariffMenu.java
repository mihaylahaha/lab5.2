...
public class TariffMenu {
    ...
    public TariffMenu(TariffRepository repository, Scanner in) {
    ...
    public void run() {
        while (true) {
            ...
            // 🔹 Обробка інших пунктів меню через if ... else if ...
            if (choice == 1) {
                createTariff();
            } else if (choice == 2) {
                deleteTariff();
    ...
    private void sortByMonthlyFee() {
        List<Tariff> sorted = new ArrayList<>(repository.all());
    ...
    private void computeTotals() {
        ...
        System.out.printf(Locale.US, "Середня абонплата: %.2f%n", avgFee);
    }
